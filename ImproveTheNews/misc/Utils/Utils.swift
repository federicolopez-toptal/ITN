...
/*
    Definition: https://improvethenews.atlassian.net/browse/ITN-4151
    - Less than a day: no conversion
    - Less than a month: Day + Month (Ex: JAN 02)
    - More than a month: Month + Year (Ex: NOV 2024)
*/
        ...
        if let _strNum = parts.first, let _num = Int(_strNum) {
            ...
            switch(_type.lowercased()) {
                case "minute", "minutes":
                    result = STRTIME_with(num: _num, typeSingular: "minute", typePlural: "minutes")
                    
                case "hour", "hours":
                    if(_num>23) {
                        let date = NOW_MINUS(hours: _num)
                        let formatter = DateFormatter()
                        formatter.calendar = Calendar(identifier: .gregorian)
...
//                        
                        ...
                        result = STRTIME_with(num: _num, typeSingular: "hour", typePlural: "hours")
                    }
