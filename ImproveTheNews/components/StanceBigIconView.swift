...
protocol StanceBigIconViewDelegate: AnyObject {
    func onStanceBigIconTap(sender: StanceBigIconView)
}

class StanceBigIconView: UIView {

    var DIM: CGFloat = 30
    var THUMB: CGFloat = 6
    
    weak var delegate: StanceBigIconViewDelegate?
    ...
    init() {        
        ...
        slider1.activateConstraints([
            slider1.widthAnchor.constraint(equalToConstant: 18),
        ...
        // ----------------
        slider2.backgroundColor = slider1.backgroundColor
        self.addSubview(slider2)
        slider2.activateConstraints([
            slider2.widthAnchor.constraint(equalToConstant: 18),
    ...
    func setValues(_ value1: Int, _ value2: Int) {
        ...
        let positions: [CGFloat] = [0, 3, 6, 9, 12]
    ...
    @objc func viewOnTap(sender: UITapGestureRecognizer?) {
        ...
        if(PREFS_SHOW_STANCE_POPUPS()) {
            self.delegate?.onStanceBigIconTap(sender: self)
        }
    }
