//
//  TopicSelectorViewPlus.swift
//  ImproveTheNews
//
//  Created by Federico Lopez on 07/08/2024.
//

import UIKit

protocol TopicSelectorViewPlusDelegate: AnyObject {
    func onTopicSelected(_ index: Int)
    func onTopicBackButtonTap()
}

class TopicSelectorViewPlus: UIView {

    weak var delegate: TopicSelectorViewPlusDelegate?
...
//        ])
    ...
    func setTopics(_ topics: [String], addBack: Bool = false) {
        MAIN_THREAD { [self] in
            ...
            if(addBack) {
                ...
                let backButtonVStack = VSTACK(into: HStack)
                
                backButtonVStack.tag = -1
                ADD_SPACER(to: backButtonVStack, height: 12)
                
                let backView = UIView()
                backView.backgroundColor = .clear
                backButtonVStack.addArrangedSubview(backView)
                backView.activateConstraints([
                    backView.widthAnchor.constraint(equalToConstant: 32),
                    backView.heightAnchor.constraint(equalToConstant: 32)
                ])
                
                let backButtonIcon = UIImageView(image: UIImage(named: DisplayMode.imageName("circle.back")))
                backView.addSubview(backButtonIcon)
                backButtonIcon.activateConstraints([
                    backButtonIcon.leadingAnchor.constraint(equalTo: backView.leadingAnchor),
                    backButtonIcon.topAnchor.constraint(equalTo: backView.topAnchor),
                    backButtonIcon.trailingAnchor.constraint(equalTo: backView.trailingAnchor),
                    backButtonIcon.bottomAnchor.constraint(equalTo: backView.bottomAnchor)
                ])
                
                let backButton = UIButton(type: .custom)
                backButton.backgroundColor = .clear //.red.withAlphaComponent(0.5)
                ...
                backButton.addTarget(self, action: #selector(self.backButtonOnTap(_:)), for: .touchUpInside)
    ...
    @objc func backButtonOnTap(_ sender: UIButton?) {
        self.delegate?.onTopicBackButtonTap()
    }
...
extension TopicSelectorViewPlus {
