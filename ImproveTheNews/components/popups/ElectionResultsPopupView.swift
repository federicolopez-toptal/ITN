//
//  ElectionResultsPopupView.swift
//  ImproveTheNews
//
...
protocol ElectionResultsPopupViewDelegate: AnyObject {
    func ElectionResultsPopupView_onUrlOpened(sender: ElectionResultsPopupView, url: String)
}


class ElectionResultsPopupView: PopupView {

    let titleLabel = UILabel()
    var descriptionLabel: HyperlinkLabel!

    weak var delegate: ElectionResultsPopupViewDelegate?
    ...
    func onLinkTap(_ url: URL) {
        ...
        self.dismissMe()
        self.delegate?.ElectionResultsPopupView_onUrlOpened(sender: self, url: url.absoluteString)
    }
